...
describe("MCP Server Smoke Tests", () => {
  ...
  test("should compress context with CPC", async () => {
    const response = await client.request("tools/call", {
      name: "compress",
      arguments: {
        context: "The quick brown fox jumps over the lazy dog. This is a test sentence. Another sentence here for compression testing. Final sentence.",
        query: "fox jumps",
        target_ratio: 0.5,
      },
    });
...
describe("MCP Tools Integration Tests", () => {
  ...
  test("think tool: multi-step reasoning chain", async () => {
    ...
    const step2 = await client.request("tools/call", {
      name: "think",
      arguments: {
        thought: "Base case: For n=1, sum = 1 and formula gives 1(1+1)/2 = 1. Therefore base case holds.",
        step: 2,
        total: 3,
        verify: true,
        domain: "math",
    ...
    const step2Data = extractJson((step2.result as { content: Array<{ text: string }> }).content[0]?.text || "");
    ...
    const step3 = await client.request("tools/call", {
      name: "think",
      arguments: {
        thought: "Inductive step: Assume true for k. Then sum(k+1) = k(k+1)/2 + (k+1) = (k+1)(k+2)/2. QED.",
        step: 3,
        total: 3,
        is_final: true,
        domain: "math",
    ...
    const step3Data = extractJson((step3.result as { content: Array<{ text: string }> }).content[0]?.text || "");
    ...
    const summaryText = (session.result as { content: Array<{ text: string }> }).content[0]?.text || "";
  ...
  test("think tool: branching exploration", async () => {
    ...
    const fullText = (session.result as { content: Array<{ text: string }> }).content[0]?.text || "";
  ...
  test("think tool: verification with different domains", async () => {
    const domains = [
      ...
      { domain: "logic", thought: "If A implies B, and A is true, then B must be true (modus ponens)" },
      { domain: "code", thought: "function factorial(n) { return n <= 1 ? 1 : n * factorial(n-1); }" },
      { domain: "general", thought: "The hypothesis requires further empirical validation through controlled experiments." },
    ];
  ...
  test("compress tool: handles edge cases", async () => {
    ...
    const singleText = (singleResponse.result as { content: Array<{ text: string }> }).content[0]?.text || "";
  ...
  test("get_session tool: different formats", async () => {
    ...
    const summaryText = (summary.result as { content: Array<{ text: string }> }).content[0]?.text || "";
    ...
    const compressedText = (compressed.result as { content: Array<{ text: string }> }).content[0]?.text || "";
  ...
  test("get_session tool: branch filtering", async () => {
    ...
    const branchText = (branchOnly.result as { content: Array<{ text: string }> }).content[0]?.text || "";
  ...
  test("list_sessions tool: reflects session state", async () => {
    ...
    const afterText = (afterCreate.result as { content: Array<{ text: string }> }).content[0]?.text || "";
    ...
    const afterClearText = (afterClear.result as { content: Array<{ text: string }> }).content[0]?.text || "";
  ...
  test("think tool: guidance and pattern detection", async () => {
    ...
    const response = await client.request("tools/call", {
      name: "think",
      arguments: {
        thought: "Therefore the answer is obviously 42, so we're done.",
        step: 1,
        total: 1,
        guidance: true,
        domain: "math",
    ...
    // Should detect overconfident or premature_conclusion
    const patterns = data?.patterns as string[];
    expect(patterns.some(p => p === "overconfident" || p === "premature_conclusion")).toBe(true);
  ...
  test("think tool: risk assessment", async () => {
    ...
    const response = await client.request("tools/call", {
      name: "think",
      arguments: {
        thought: "Obviously, if we assume all values are never negative, then therefore the answer is clearly 100.",
        step: 1,
        total: 1,
        guidance: true,
        session_id: sessionId,
